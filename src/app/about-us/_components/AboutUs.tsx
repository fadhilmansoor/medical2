...
function AboutUs() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      name: String(fd.get("dzName") || "").trim(),
      email: String(fd.get("dzEmail") || "").trim(),
      phone: String(fd.get("dzPhoneNumber") || "").trim(),
      service: selectCat,
      message: String(fd.get("dzMessage") || "").trim(),
    };

    // ✅ Validation + toast
    if (!payload.name) return toast.error("Please enter your name");
    if (!payload.email) return toast.error("Please enter your email");
    if (!payload.phone) return toast.error("Please enter your phone number");
    if (!payload.service) return toast.error("Please select a service");

    const toastId = toast.loading("Sending appointment...");
    ...
    try {
      ...
      if (res.ok && data?.success) {
        toast.success(data?.message || "Appointment sent successfully!", { id: toastId });
        form.current.reset();
        setSelectCat("Angioplasty");
      } else {
        toast.error(data?.message || "Failed to send appointment.", { id: toastId });
      }
    } catch (err) {
      console.error(err);
                    ...
                    <form ref={form} onSubmit={handleSubmit} className="dzForm">
                      <div className="row">
                        ...
                        <div className="col-sm-12 m-b30">
                          <div className="form-floating floating-underline input-light">
                            <textarea name="dzMessage" className="form-control" id="inputMessage" rows={6} placeholder="Message (Optional)" />
                            <label htmlFor="inputMessage">Message (Optional)</label>
                          </div>
                        </div>
