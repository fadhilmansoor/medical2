...
function markdownToHtml(markdown: string): string {
  ...
  html = html.replace(/^- (.*$)/gim, "<li>$1</li>");
  html = html.replace(/(<li>.*<\/li>)/s, '<ul class="dz-list">$1</ul>');

  html = html.replace(/^\d+\.\s(.*$)/gim, "<li>$1</li>");

  html = html.replace(/^---$/gim, '<hr class="wp-block-separator" />');

  html = html.replace(/```(.*?)```/gis, '<pre class="code-block"><code>$1</code></pre>');
  html = html.replace(/`(.*?)`/gim, '<code class="inline-code">$1</code>');

  const lines = html.split("\n");
  const processedLines = lines.map((line) => {
    const t = line.trim();
    if (!t || t.match(/^<(h[1-6]|blockquote|ul|ol|li|pre|hr|img)/)) return line;
